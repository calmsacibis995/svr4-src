...
#ident	"@(#)mbus:cmd/mbusutils/mb2.d/cci.d/bin.d/ccifree.c	1.3"

static char ccifree_copyright[] = "Copyright 1987 Intel Corp. 461772";
...

{
	...
	req_buf.free_req.line_disc_id = line_disc;
	req_buf.buf[0] = CCI_FREE;

	actual = cci_send_cmd(CCI_PORTID, &req_buf, &free_reply_buf, NULL, 0L,
							NULL, 0L, &status);
	if (status != E_OK) {
		printf("CCI SEND COMMAND Error : %x\n", status);
		myexit(1);
	}
	if (free_reply_buf.status != E_OK) {
		printf("FREE Error : %x\n", free_reply_buf.status);
		myexit(1);
	}

	printf("FREEd Up Line Discipline %d\n", line_disc);
